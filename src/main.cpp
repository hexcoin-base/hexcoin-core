...
#if defined(NDEBUG)
# error "HexCoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x38203f7a310f5ae1765f6d65472a150dc45e4fbcdb2c176f82c42115d1563b22");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // HexCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "HexCoin Signed Message:\n";
...
{
    // HexCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // HexCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // HexCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1512916520, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "Spiegel Online 12122017 Erdogan bezeichnet Israel als Terrorstaat";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04a3cbbdd962172e8da3c197d310018a1290c1891db24ed312f43d03642ff62acdefa87d1863e0ea97dabec3bff55a4bc8ecf47182e14ae17034c20431e24ab763") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1512916520;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1252025;
        ...
        assert(block.hashMerkleRoot == uint256("0x83e1e13e5a0fd3fa4e5a07abe76dc7d167fa8b86a90132d954de4ee57d70dc41"));
...
unsigned char pchMessageStart[4] = { 0x18, 0x3c, 0x55, 0x8b }; // HexCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("HexCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("HexCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static HexCoinMiner(CWallet *pwallet)
{
    printf("HexCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("hexcoin-miner");
    ...
    try { loop {
        ...
        printf("Running HexCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("HexCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&HexCoinMiner, pwallet));
}
